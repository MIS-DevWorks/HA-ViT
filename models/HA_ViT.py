...
class HA_ViT(nn.Module):
    ...
    def __init__(self, img_size=112, patch_size=8, in_chans=3, embed_dim=1024, num_classes_list=(2239, 7, 2),
        ...
        self.patch_embed_ocu = PatchEmbed(
            img_size=img_size, view=2, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
    ...
    def forward(self, x_face, x_ocular, return_feature=False):
        x_face = self.forward_patch_embed(x_face, mode="face")
        x_face = self.forward_features(x_face)

        x_ocular = self.forward_patch_embed(x_ocular, mode="ocular")
